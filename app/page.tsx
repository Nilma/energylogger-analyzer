    ...
    <main className="shell">
      <section className="hero">
        ...
        <p className="intro">
          ...
          reports energy consumption in joules.
        </p>
      </section>
      ...
      <section className="card">
        <form onSubmit={handleSubmit}>
          <label className="upload">
            <span>{file ? file.name : "Choose a CSV log file"}</span>
            <input
              type="file"
              accept=".csv,text/csv"
              onChange={(event) => {
        ...
        <>
          <section className="summary">
            <article className="metric">
              <span>Duration</span>
              <strong>{result.duration_s.toFixed(1)} s</strong>
            </article>
            <article className="metric">
              <span>Average power</span>
              <strong>{result.average_power_w.toFixed(2)} W</strong>
            </article>
            <article className="metric">
              <span>Total energy</span>
              <strong>{result.total_energy_j.toFixed(1)} J</strong>
            </article>
          </section>
          ...
          <section className="card">
            ...
            <div className="tableWrap">
              <table>
                ...
                <tbody>
                  {Object.entries(result.energy_by_rail_j)
                    .sort((a, b) => b[1] - a[1])
                    .map(([rail, energy]) => (
                      <tr key={rail}>
                        <td>{rail}</td>
                        ...
                        <td>
                          {result.total_energy_j > 0
                            ? ((energy / result.total_energy_j) * 100).toFixed(1)
                            : "0.0"}
                          %
                        </td>
                      </tr>
